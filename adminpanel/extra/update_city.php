...
$data = $general->getSingdet(" city_c where id_c = ".$_GET['id']);
?>
<!DOCTYPE html>
<html>
              ...
              <div class="box box-primary">
                ...
                <form name="addform" method="post" action="" enctype="multipart/form-data">
                  ...
                  <? if(is_array($coutry)) {?>
                      ...
                      <select name="upd_idcou_c" class="form-control">
                     <? for($a=0;$a<count($coutry);$a++){?>
                      <option value="<?=$coutry[$a]['id_cou'];?>" <? if($coutry[$a]['id_cou']==$data[0]['idcou_c']){ echo "selected"; }?>><?=$coutry[$a]['name_cou'];?></option>
                      <? }?>
                      </select>
                     </div>
                      <? }?>
                  	  ...
                  	  <div class="col-xs-4">
                      <label>Name</label>
                      <input type="text" class="form-control" name="upd_name_c" value="<?=$data[0]['name_c']?>" />
                      </div>
                      ...
                      <div class="col-xs-4">
                      <label>Status</label>
                      <select class="form-control" name="upd_status_c">
                      <option value="1" <? if($data[0]['status_c']==1) echo 'selected'; ?>>Active</option>
                      <option value="0" <? if($data[0]['status_c']==0) echo 'selected'; ?>>Inactive</option>
                      </select>
                      </div>
                      
                      <div class="col-xs-4">
                      <label>Order</label>
                      <input type="text" class="form-control"  name="upd_order_c" value="<?=$data[0]['order_c']?>" />
                      </div>
