...
if(isset($_GET['id'])){
	$general->delGenDetails("ccity_cty","id_cty","",$conf->absolute_path.$conf->banner_dir);
}
if(isset($_POST['sortion'])){
	$chk = $general->sortDataGeneral("ccity_cty","order_cty","id_cty");	
}
if(isset($_GET['status'])){
	$_GET['st_id'] = base64_decode($_GET['st_id']);
	if($_GET['status']=='Inactive'){
		
		$qry= "UPDATE ccity_cty set status_cty='0' where id_cty=".$_GET['st_id'];
		$general->CustomModify($qry);
	 
	} 
	else{
		 
		 $qry1= "UPDATE ccity_cty set status_cty='1' where id_cty=".$_GET['st_id'];
		 $general->CustomModify($qry1);		
		
	}
}
$optbyid=$general->getAll(" ccity_cty ");
?>
<!DOCTYPE html>
<html>
              ...
              <div class="box">
                ...
                <? if(is_array($optbyid)){ ?>
                  ...
                  <table id="example1" class="table table-bordered table-striped">
					...
					$child=$general->getAll(" ccity_cty where parent_cty = ".$optbyid[$d]['id_cty']."");
					?>
                    	<tr>                        	
                            <td><? if(is_array($child)){?><a href="manage_subcty.php?cid=<?=base64_encode($optbyid[$d]['id_cty'])?>"><?=$optbyid[$d]['name_cty']?></a><? }else{ ?><?=$optbyid[$d]['name_cty']?><? } ?></td>
                            <td><? if($optbyid[$d]['status_cty']==1)
				   { ?><a href='?status=Inactive&st_id=<?=base64_encode($optbyid[$d]['id_cty'])?>'>Active</a><? }  else { ?><a href='?status=Active&st_id=<?=base64_encode($optbyid[$d]['id_cty'])?>'>Inactive</a><? } ?></td>
                            <td><input type="text" size="1" name="ordsort_<?php echo $optbyid[$d]['id_cty'];?>" value="<?=$optbyid[$d]['order_cty']?>" /></td>
                            <td style="text-align:center !important;">
                            <?php /*?><a href="#" class="view" id="<?php echo $optbyid[$d]['id_cty']?>" ><img src="dist/img/icons/view.png" alt="Edit" /></a><?php */?>
                            <a href="update_<?=$pagetype?>.php?id=<?php echo base64_encode($optbyid[$d]['id_cty'])?>" target="_self" title="Edit"><img src="dist/img/icons/pencil.png" alt="Edit" /></a>
                            <a href="#" class="delete" id="<? echo $optbyid[$d]['id_cty'];?>"><img src="dist/img/icons/cross.png" alt="Delete" /></a>
                            </td>
                        </tr>
                    <? } ?>  
                    </tbody>
