...
if(isset($_GET['id'])){
	$general->delGenDetails("product_reviews","id","",$conf->absolute_path.$conf->banner_dir);
}
if(isset($_POST['sortion'])){
	$chk = $general->sortDataGeneral("product_reviews","order","id");	
}
if(isset($_GET['status'])){
	$_GET['st_id'] = base64_decode($_GET['st_id']);
	if($_GET['status']=='Inactive'){
		
		$qry= "UPDATE product_reviews set status='0' where id=".$_GET['st_id'];
		$general->CustomModify($qry);
	 
	} 
	else{
		 
		 $qry1= "UPDATE product_reviews set status='1' where id=".$_GET['st_id'];
		 $general->CustomModify($qry1);		
		
	}
}
$optbyid=$general->getAll(" product_reviews");
?>
<!DOCTYPE html>
<html>
                ...
                <? if(is_array($optbyid)){ ?>
                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <? for($d=0; $d<count($optbyid); $d++){?>
                    	<tr>                        	
                            <td><?=$optbyid[$d]['name']?></td>
                            <td><?=$optbyid[$d]['email']?></td>
                            <td><? if($optbyid[$d]['status']==1)
				   { ?><a href='?status=Inactive&st_id=<?=base64_encode($optbyid[$d]['id'])?>'>Active</a><? }  else { ?><a href='?status=Active&st_id=<?=base64_encode($optbyid[$d]['id'])?>'>Inactive</a><? } ?></td>
                            <td><?=$optbyid[$d]['datetime']?></td>
                            <td style="text-align:center !important;">
                            <a href="#" class="view" id="<?php echo $optbyid[$d]['id']?>" ><img src="dist/img/icons/view.png" alt="Edit" /></a>
                            <?php /*?><a href="update_<?=$pagetype?>.php?id=<?php echo base64_encode($optbyid[$d]['id'])?>" target="_self" title="Edit"><img src="dist/img/icons/pencil.png" alt="Edit" /></a><?php */?>
                            <a href="#" class="delete" id="<? echo $optbyid[$d]['id'];?>"><img src="dist/img/icons/cross.png" alt="Delete" /></a>
                            </td>
                        </tr>
                    <? } ?>  
                    </tbody>
