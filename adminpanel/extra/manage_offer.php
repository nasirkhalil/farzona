...
if(isset($_GET['id'])){
	$general->delGenDetails("offers_ofr","id_ofr","",$conf->absolute_path.$conf->general_dir);
}
if(isset($_POST['sortion'])){
	$chk = $general->sortDataGeneral("offers_ofr","order_ofr","id_ofr");	
}
if(isset($_GET['status'])){
	$_GET['st_id'] = base64_decode($_GET['st_id']);
	if($_GET['status']=='Inactive'){
		
		$qry= "UPDATE offers_ofr set status_ofr='0' where id_ofr=".$_GET['st_id'];
		$general->CustomModify($qry);
	 
	} 
	else{
		 
		 $qry1= "UPDATE offers_ofr set status_ofr='1' where id_ofr=".$_GET['st_id'];
		 $general->CustomModify($qry1);		
		
	}
}
$optbyid=$general->getAll(" offers_ofr ");
?>
<!DOCTYPE html>
<html>
                ...
                <? if(is_array($optbyid)){ ?>
                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <? for($d=0; $d<count($optbyid); $d++){?>
                    	<tr>                        	
                            <td><?=$optbyid[$d]['name_ofr']?></td>
                            <td><? if($optbyid[$d]['status_ofr']==1)
				   { ?><a href='?status=Inactive&st_id=<?=base64_encode($optbyid[$d]['id_ofr'])?>'>Active</a><? }  else { ?><a href='?status=Active&st_id=<?=base64_encode($optbyid[$d]['id_ofr'])?>'>Inactive</a><? } ?></td>
                            <td><input type="text" size="1" name="ordsort_<?php echo $optbyid[$d]['id_ofr'];?>" value="<?=$optbyid[$d]['order_ofr']?>" /></td>
                            <td style="text-align:center !important;">
                            <?php /*?><a href="#" class="view" id="<?php echo $optbyid[$d]['id_ofr']?>" ><img src="dist/img/icons/view.png" alt="Edit" /></a><?php */?>
                            <a href="update_<?=$pagetype?>.php?id=<?php echo base64_encode($optbyid[$d]['id_ofr'])?>" target="_self" title="Edit"><img src="dist/img/icons/pencil.png" alt="Edit" /></a>
                            <a href="#" class="delete" id="<? echo $optbyid[$d]['id_ofr'];?>"><img src="dist/img/icons/cross.png" alt="Delete" /></a>
                            </td>
                        </tr>
                    <? } ?>  
                    </tbody>
