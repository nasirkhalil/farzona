...
$where = " Where 1 ";
if($_GET['parent_id'] > 0){
  $where .= " and parent_id=".$_GET['parent_id']; 
}else{
  $where .= " and parent_id=0"; 
}
$optbyid = $general->getAll(" size_details  $where");
?>
<!DOCTYPE html>
<html>
                ...
                <? if(is_array($optbyid)){ ?>
                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <? for($d=0; $d<count($optbyid); $d++){?>
                    	<tr>
                        	<td><input type="checkbox" name="checkbox[]" class="case" value="<? echo $optbyid[$d]['id'];?>" /></td>  
                          <td><? echo $general->getSingleField("size_type","id",$optbyid[$d]['size_type_id'],"name")?></td> 
                          <?php if( $general->getSingleField("size_details","parent_id",$optbyid[$d]['id'],"parent_id") > 0 ){ ?>                     	
                          <td><a href="?parent_id=<?php echo $optbyid[$d]['id'] ?>"><?=$optbyid[$d]['value']?></a></td>
                          <?php }else{ ?>
                            <td><?=$optbyid[$d]['value']?></td>
                          <?php } ?>
				   ...
				   { ?><a href='?status=Inactive&st_id=<?=base64_encode($optbyid[$d]['id'])?>'>Active</a><? }  else { ?><a href='?status=Active&st_id=<?=base64_encode($optbyid[$d]['id'])?>'>Inactive</a><? } ?></td>
                            ...
                            <? } ?><?php */?>
                            <!--Edit-->
                            <a href="update_<?=$pagetype?>.php?id=<?php echo base64_encode($optbyid[$d]['id'])?>" target="_self" title="Edit"><img src="dist/img/icons/pencil.png" alt="Edit" /></a>
                            <!--Delete-->
