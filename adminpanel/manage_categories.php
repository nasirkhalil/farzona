...
if(isset($_GET['cid'])){
	...
	$optbyid = $general->getAll(" category_cat WHERE parent_cat = 0  ORDER BY order_cat ");
}
?>
<!DOCTYPE html>
<html>
                ...
                <? if(is_array($optbyid)){ ?>
                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <? for($d=0; $d<count($optbyid); $d++){ 
                        $child = $general->getAll(" category_cat where parent_cat = ".$optbyid[$d]['id_cat']." order by order_cat "); ?>
                    	<tr>
                        	<td><input type="checkbox" name="checkbox[]" class="case" value="<? echo $optbyid[$d]['id_cat'];?>" /></td>                        	
                            <td><?php /*?><img src="<?=$conf->site_url.$conf->category_dir.$optbyid[$d]['thumb_name_cat']?>" width="20%" /><?php */?>
                             <? if(is_array($child)){?>
                             <a href="manage_categories.php?cid=<?=base64_encode($optbyid[$d]['id_cat'])?>"><?=$optbyid[$d]['name_cat']; if($conf->ar==1){ echo " | ".$optbyid[$d]['namear_cat'];}  ?></a>
                             <? }else{ ?>
