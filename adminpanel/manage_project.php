...
$optbyid = $general->getAll("product_prd where type_prd = 'project' order by order_prd ");
?>
<!DOCTYPE html>
<html>
            ...
            <div class="col-xs-12">
              <div class="box">
                ...
                <? 	   }
                  ...
                  <h3 class="box-title" style="float:right !important;"><a href="add_products.php?type=<?=$prdtype?>">Add Project</a></h3>
                </div><!-- /.box-header -->
                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <? for($d=0; $d<count($optbyid); $d++){?>
                    	<tr>
                            ...
                            <td><img src="<?=$conf->site_url.$conf->product_dir."_".$optbyid[$d]['thumb_name_prd']?>" class="img-responsive" />
							<?=$optbyid[$d]['name_prd']?></td>
                            <td><? if($optbyid[$d]['status_prd']==1)
				   { ?><a href='?status=Inactive&st_id=<?=base64_encode($optbyid[$d]['id_prd'])?>&type=<?=$_GET['type']?>'>Active</a><? }  else { ?><a href='?status=Active&st_id=<?=base64_encode($optbyid[$d]['id_prd'])?>&type=<?=$_GET['type']?>'>Inactive</a><? } ?></td>
                           ...
                           <? if($prdtype=='project'){?>
                            ...
                            <? } ?>
                            <!--Edit-->
                            <a href="update_products.php?id=<?php echo base64_encode($optbyid[$d]['id_prd'])?>" target="_self" title="Edit"><img src="dist/img/icons/pencil.png" alt="Edit" /></a>
                            <!--Delete-->
