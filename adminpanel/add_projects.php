...
if($_POST['button'] != "" || $_POST['button'] != NULL){
	$chk=$general->insertProductsMulti("product_prd");
}
?>
<!DOCTYPE html>
<html>
...
<? $cats = $general->getAll(" category_cat where status_cat = '1' order by order_cat ");?>                      
                      ...
                      <select name="idcat_prd" class="form-control">
                      <? if(is_array($cats)){
					  for($c=0; $c<count($cats); $c++){?>
                      	<option value="<?=$cats[$c]['id_cat']?>"><?=$cats[$c]['name_cat']?></option>
                      <? } }else{ ?>
...
var settings = {
	url: "<?=$conf->site_url?>multiuploader/uploadprojects.php",
	method: "POST",
	...
	{		
		$("#status").html("");
		$("#piclist").append("<div id='a_"+$.trim(data)+"'><span width='200'><img src='<?=$conf->site_url.$conf->product_dir?>"+$.trim(data)+"' width='100' height='100' /><br />"+$.trim(data)+"<div class='delit'><img src='../multiuploader/upload-cancel.png'/></div></span><input type='hidden' name='modpics[]' value='"+$.trim(data)+"' /><input type='text' class='form-control' name='caption[]' placeholder='Caption' /><br /><input type='text' name='alt[]' class='form-control' placeholder='Image Alt' /><br /><input type='text' name='title[]' class='form-control' placeholder='Image Title' /><br /><input type='text' class='form-control' name='order[]' placeholder='Order' /><br /></div>");	
		allimgs += $.trim(data)+",";
...
$("body").on("click",".delit", function(sds){
	...
	$.post("../multiuploader/delpics.php",{ pic: pic, path:'<?=$conf->absolute_path.$conf->product_dir?>' }, function(data) {
		if(data!=""){
			var b = $('div[id="a_'+pic+'"]');
			b.empty();
			//alert($.trim(data));
