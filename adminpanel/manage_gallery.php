...
if(isset($_REQUEST['multidelete'])){
	$imgfields = array('image_gal');
	$chk = $general->delmulti("gallery_gal","id_gal",$conf->absolute_path.$conf->gallery_dir,$imgfields);
	echo $chk; exit();	
}
/*single deletion*/
if(isset($_GET['id'])){
	$imgfields = array('image_gal');
	$chk = $general->delGenDetails("gallery_gal","id_gal",$conf->absolute_path.$conf->gallery_dir,$imgfields);
	echo $chk; exit();
}
/*sortion*/
if(isset($_POST['sortion'])){
	$chk = $general->sortDataGeneral("gallery_gal","order_gal","id_gal");	
}
/*status updation*/
if(isset($_GET['status'])){
	$_GET['st_id'] = base64_decode($_GET['st_id']);
	if($_GET['status']=='Inactive'){
		$general->UpdStatus("gallery_gal","id_gal","status_gal",0,$_GET['st_id']);		
	} 
	else{
		 $general->UpdStatus("gallery_gal","id_gal","status_gal",1,$_GET['st_id']);
	}
}
/*status updation ends */
/* get all banners by origin */
$optbyid = $general->getAll(" gallery_gal ");
?>
<!DOCTYPE html>
<html>
                ...
                <? if(is_array($optbyid)){ ?>
                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <? for($d=0; $d<count($optbyid); $d++){?>
                    	<tr>
                        	<td><input type="checkbox" name="checkbox[]" class="case" value="<? echo $optbyid[$d]['id_gal'];?>" /></td>                        	
                            <td><img src="<?=$conf->site_url.$conf->gallery_dir.$optbyid[$d]['image_gal']?>" width="30%" /></td>
                            <td><? if($optbyid[$d]['status_gal']==1)
				   { ?><a href='?status=Inactive&st_id=<?=base64_encode($optbyid[$d]['id_gal'])?>'>Active</a><? }  else { ?><a href='?status=Active&st_id=<?=base64_encode($optbyid[$d]['id_gal'])?>'>Inactive</a><? } ?></td>
                            <td><input type="text" size="1" name="ordsort_<?php echo $optbyid[$d]['id_gal'];?>" value="<?=$optbyid[$d]['order_gal']?>" /></td>
                            <td style="text-align:center !important;">
                            <a href="update_<?=$pagetype?>.php?id=<?php echo base64_encode($optbyid[$d]['id_gal'])?>" target="_self" title="Edit"><img src="dist/img/icons/pencil.png" alt="Edit" /></a>
                            <a href="#" class="delete" id="<? echo $optbyid[$d]['id_gal'];?>"><img src="dist/img/icons/cross.png" alt="Delete" /></a>
                            </td>
                        </tr>
                    <? } ?>  
                    </tbody>
