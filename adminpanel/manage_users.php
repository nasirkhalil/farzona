...
if(isset($_GET['id'])){
	$general->delGenDetails("user_usr","id_s","",$conf->absolute_path.$conf->banner_dir);
}
if(isset($_POST['sortion'])){
	$chk = $general->sortDataGeneral("user_usr","order_s","id_s");	
}
if(isset($_GET['status'])){
	$_GET['st_id'] = base64_decode($_GET['st_id']);
	if($_GET['status']=='Inactive'){
		
		$qry= "UPDATE user_usr set status_usr='0' where id_usr=".$_GET['st_id'];
		$general->CustomModify($qry);
	 
	} 
	else{
		 
		 $qry1= "UPDATE user_usr set status_usr='1' where id_usr=".$_GET['st_id'];
		 $general->CustomModify($qry1);		
		
	}
}
$optbyid=$general->getAll(" user_usr ");
?>
<!DOCTYPE html>
<html>
            ...
            <div class="col-xs-12">
              <div class="box">
                ...
                <? 	   }
                  ...
                  <?php /*?><h3 class="box-title" style="float:right !important;"><a href="add_<?=$pagetype?>.php">Add <?=$pagetype?></a></h3><?php */?>
                </div><!-- /.box-header -->
                <div class="box-body">
                <? if(is_array($optbyid)){ ?>
                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <? for($d=0; $d<count($optbyid); $d++){?>
                    	<tr>                        	
                            <td><?=$optbyid[$d]['first_name_usr']." ".$optbyid[$d]['last_name_usr']?></td>
                            <td><?=$optbyid[$d]['email_usr']?></td>
                            <td><? if($optbyid[$d]['status_usr']==1)
				   { ?><a href='?status=Inactive&st_id=<?=base64_encode($optbyid[$d]['id_usr'])?>'>Active</a><? }  else { ?><a href='?status=Active&st_id=<?=base64_encode($optbyid[$d]['id_usr'])?>'>Inactive</a><? } ?></td>
                            <td><input type="text" size="1" name="ordsort_<?php echo $optbyid[$d]['id_usr'];?>" value="<?=$optbyid[$d]['order_usr']?>" /></td>
                            <td style="text-align:center !important;">
                            <?php /*?><a href="#" class="view" id="<?php echo $optbyid[$d]['id_usr']?>" ><img src="dist/img/icons/view.png" alt="Edit" /></a><?php */?>
                            <a href="update_<?=$pagetype?>.php?id=<?php echo base64_encode($optbyid[$d]['id_usr'])?>" target="_usrelf" title="Edit"><img src="dist/img/icons/pencil.png" alt="Edit" /></a>
                            <a href="#" class="delete" id="<? echo $optbyid[$d]['id_usr'];?>"><img src="dist/img/icons/cross.png" alt="Delete" /></a>
                            </td>
                        </tr>
                    <? } ?>  
                    </tbody>
